...
class DownloadThread(QThread):
    ...
    def __init__(self, playlist_url, resolution, base_save_folder):
        ...
        self.total_bytes = 0  # Total size of playlistt
    ...
    def run(self):
        try:
            ...
            # Calculate total size of all videos in the playlist
            for video_url in videos:
                yt = YouTube(video_url)
                video = yt.streams.filter(file_extension="mp4", resolution=self.resolution).first()
                self.total_bytes += video.filesize

            playlist_folder_name = self.playlist_url.split('=')[-1]  # Extract playlist ID for folder name
            save_folder = os.path.join(self.base_save_folder, playlist_folder_name)
            os.makedirs(save_folder, exist_ok=True)  # Create a folder for the playlist if it doesn't exist

            for video_url in videos:
                yt = YouTube(video_url)
                video = yt.streams.filter(file_extension="mp4", resolution=self.resolution).first()

                save_path = os.path.join(save_folder, f'{yt.title}.mp4')
...
class YouTubeDownloaderApp(QWidget):
    ...
    def update_total_progress(self, total_progress):
