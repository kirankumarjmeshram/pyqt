...
class ConfigPage(QWidget):
    ...
    def initUI(self):
        ...
        add_button.clicked.connect(self.main)  # connecting main
    ...
    def main(self):  # Added this method
        ...
        logger.configure(
            handlers=[
                {"sink": sys.stderr, "level": "CRITICAL"},
                {"sink": sys.stdout, "level": "CRITICAL"},
            ]
        )

        logger.add(
            os.path.join(os.path.expanduser("~"), "triage_preprocessor_service.log"),
            rotation="500 MB",
        )  
